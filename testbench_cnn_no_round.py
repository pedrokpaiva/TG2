...
def testbench_run(ckt=None, results_filename=None):
    ...
    # Train the model
    if ( not os.path.isfile( "data/net_quantized_no_round.pt")):
        ...
        # Save model
        torch.save(net_quantized.state_dict(), "data/net_quantized_no_round.pt")
    else:
        net_quantized = torch.ao.quantization.convert(net_quantized)
        net_quantized.load_state_dict(torch.load("data/net_quantized_no_round.pt"))
        net_quantized.eval()
