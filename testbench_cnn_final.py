...
class VerySimpleNet(nn.Module):
    ...
    def forward(self, x, ckt=None, wgt_tensor=None, results_filename=None, test = 0):
        ...
        if(test):
            ...
            # x = x.repeat(10, 1, 1, 1)
            # x = self.quant(x.to(torch.float32))
        else:
            x = self.conv1(x)
            x = self.ReLu1(x)
...
def testbench_run(ckt=None, results_filename=None):
    ...
    # Train the model
    if (not os.path.isfile( "data/net_quantized.pt")):
...
def run_rtl(ckt=None, ifm_tensor=None,  wgt_tensor=None, results_filename=None):
    ...
    ifm_np = torch.int_repr(ifm_tensor.data).numpy()
    # plt.imshow(torch.int_repr(ifm_tensor[0][0]), cmap='gray')
    # plt.title("ifm_tensor")
    # plt.show()
    # input()
                    ...
                    for j in range(TI + 3):
                        ...
                        for i in range(8):
                            ...
                            if ((row < 64) and (col < 64)):
                                ...
                                data = 0
                            if(i < 4):
                                ifm_in_msb.append(BitArray(int8=data))
                            else:
                                ifm_in_lsb.append(BitArray(int8=data))
    ...
    quantized_tensor = torch.quantize_per_tensor(tensor, 0.04704698547720909, 0, torch.quint8)
