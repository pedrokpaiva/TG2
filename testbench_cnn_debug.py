...
class VerySimpleNet(nn.Module):
    ...
    def forward(self, x, ckt=None, wgt_tensor=None, results_filename=None, test = 0):
        ...
        if(test):
            node_info, rtl_out = run_rtl(ckt, x, wgt_tensor, results_filename)
            # y = self.conv1(x)
            # python_out = self.ReLu1(y)
            x = torch.unsqueeze(rtl_out, 0)
            # torch.set_printoptions(profile="full")

            # print(x[0][0])
            # print("rtl_out")
            # input()
            # print(python_out[0][0])
            # print("python_out")
            # input()
            # print(torch.int_repr(python_out[0][0]))
            # print("python_out int")
            # input()
            # torch.set_printoptions(profile="default")
...
def testbench_run(ckt=None, results_filename=None):
    ...
    # Train the model
    if ( not os.path.isfile( "data/net_quantized_debug.pt")):
        ...
        # Save model
        torch.save(net_quantized.state_dict(), "data/net_quantized_debug.pt")
    else:
        net_quantized = torch.ao.quantization.convert(net_quantized)
        net_quantized.load_state_dict(torch.load("data/net_quantized_debug.pt"))
        net_quantized.eval()
    ...
    wgt_tensor = net_quantized.conv1.weight()
...
def run_rtl(ckt=None, ifm_tensor=None,  wgt_tensor=None, results_filename=None):
    ...
    while (not stop):
        ...
        if (oc <= (OFM_C - 1)):
            ...
            if (stop_flag):
                with open("data/output_rtl_debug.txt", "w") as f:
                    for toc in range(0, OFM_C):
                        for toh in range(0, OFM_H):
                            ...
                            for tow in range(0, OFM_W):
                                ...
                                f.write(s)
                                bit_array = BitArray(uint=ofm[toc][toh+buffer][tow], length=32)
                                bit_array >>=11
    ...
    tensor = tensor.mul(0.049549445509910583)
    # torch.set_printoptions(profile="full")
    # print(tensor[0])
    # print("tensor float pre quantizacao")
    # input()
    # torch.set_printoptions(profile="default")
    ...
    # print("tensor post quantizacao")
    # input()
    # print(torch.int_repr(quantized_tensor[0]))
    # print("tensor post quantizacao int")
    # input()
    # torch.set_printoptions(profile="default")
